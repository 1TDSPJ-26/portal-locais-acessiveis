  ...
  return (
    <main className="app-shell">
      <header className="hero">
        ...
        <p className="hero-copy">Pesquise por nome ou combine recursos de acessibilidade para planejar sua próxima saída.</p>
      ...
      <section className="content" aria-label="Locais acessíveis">
        <div className="results-heading">
          <div>
            ...
            <h2>{resultados.length} {resultados.length === 1 ? 'resultado encontrado' : 'resultados encontrados'}</h2>
          </div>
          <button
          ...
          >
            ...
            {filtros.recursos.length + (filtros.categoria ? 1 : 0) > 0 && (
              <span className="filter-count">{filtros.recursos.length + (filtros.categoria ? 1 : 0)}</span>
            )}
          </button>
        </div>

        <aside id="filtros-locais" className={`filters-panel ${painelAberto ? 'is-open' : ''}`}>
          <div className="filters-topline">
            <div>
              <p className="section-kicker">Refine sua busca</p>
              <h2>O que você precisa?</h2>
            </div>
            <button className="clear-button" type="button" onClick={limparFiltros} disabled={!temFiltrosAtivos}>
              Limpar filtros
            </button>
          </div>
          ...
          <fieldset>
            ...
            <div className="category-options">
              {categoriasLocais.map((categoria) => (
                <label className={`category-option ${filtros.categoria === categoria ? 'is-selected' : ''}`} key={categoria}>
                  <input
                    type="radio"
                    name="categoria"
                    value={categoria}
                    checked={filtros.categoria === categoria}
                    onChange={(evento) => setFiltros((atual) => ({ ...atual, categoria: evento.target.value as CategoriaLocal }))}
                  />
                  <span>{categoria}</span>
                </label>
              ))}
          ...
          <fieldset>
            <legend>Recursos de acessibilidade <span>(selecione todos que precisa)</span></legend>
            <div className="resource-options">
              {recursosAcessibilidade.map((recurso) => (
                <label className="check-option" key={recurso}>
                  <input type="checkbox" checked={filtros.recursos.includes(recurso)} onChange={() => alternarRecurso(recurso)} />
        ...
        {temFiltrosAtivos && (
          <div className="active-filters" aria-label="Filtros ativos">
            ...
            {termo.trim() && <button type="button" onClick={() => setTermo('')}>Busca: “{termo}” ×</button>}
            {filtros.categoria && <button type="button" onClick={() => setFiltros((atual) => ({ ...atual, categoria: '' }))}>{filtros.categoria} ×</button>}
            {filtros.recursos.map((recurso) => <button type="button" key={recurso} onClick={() => alternarRecurso(recurso)}>{recurso} ×</button>)}
          </div>
        )}
        ...
        {resultados.length > 0 ? (
          <div className="places-grid">
            {resultados.map((local) => (
              <article className="place-card" key={local.id}>
                <div className="place-card-top"><span className="place-category">{local.categoria}</span><span className="place-status">Aberto hoje</span></div>
                <h3>{local.nome}</h3>
                <p className="place-address">{local.endereco}</p>
                <div className="resource-tags" aria-label="Recursos disponíveis">
                  {local.recursos.map((recurso) => <span key={recurso}>{recurso}</span>)}
                </div>
              </article>
            ))}
          </div>
          ...
          <div className="empty-state">
            ...
            <button className="clear-button prominent" type="button" onClick={limparFiltros}>Limpar filtros</button>
          </div>
        )}
      </section>
    </main>
